<template>
  ...
  <el-form :model="productCate"
      ...
      <el-form-item label="分类图标：">
        <add-carousel ref="AddCarousel" @input="input" :value="(productCate.icon+'')"></add-carousel>
      </el-form-item>
...
<script>
  ...
  import AddCarousel from '@/views/product/productCate/components/ImageUploaderPlus'
  ...
  export default {
    ...
    created() {
      if (this.isEdit) {
        getProductCate({id:this.$route.query.id}).then(response => {
          this.productCate = response.resultParam.productSort;
        });
    ...
    methods: {
      input(res) {
        this.productCate.icon = res
      },
      getSelectProductCateList() {
        fetchList(0, {pageSize: 100, pageNum: 1}).then(response => {
          this.selectProductCateList = response.resultParam.productSortList;
          ...
          if (valid) {
              if (this.isEdit) {
                ...
                updateProductCate({
                  ...
                  description:this.productCate.description,
                  icon:this.productCate.icon,
                  keywords:this.productCate.keywords,
                  name:this.productCate.name,
                  navStatus:this.productCate.navStatus,
                  parentId:this.productCate.parentId,
                  productUnit:this.productCate.productUnit,
                  showStatus:this.productCate.showStatus,
                  sort:this.productCate.sort,
                }).then(response => {
                  this.$router.back();
                });
              } else {
